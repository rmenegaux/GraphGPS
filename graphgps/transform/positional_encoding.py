...
class SPDE(BaseAttentionPE):
    '''
    Shortest path distance encoding
    '''

    def __init__(self, **parameters):
        self.max_steps = parameters.get('max_steps', 16)
        super().__init__(**parameters)
    ...
    def compute_attention_pe(self, graph):
        ...
        A = to_dense_adj(graph.edge_index).squeeze().long()
        ...
        for step in range(1, self.max_steps):
            ...
            if (S > 0).all():
                break
        # Finally add special integers for the diagonal and for non-connected pairs
        S.fill_diagonal_(self.max_steps + 1)
        S += (S == 0) * (self.max_steps + 2)
...
def compute_shortest_paths(data, max_length=6):
