...
function run_repeats {
    ...
    #    eval $script
    #done
    script="sbatch -J ${cfg_suffix}-${dataset} -C v100-32g --ntasks=1 --cpus-per-task=200 --error=/gpfswork/rech/tbr/ump88gx/EJ_logs/%j.out --output=/gpfswork/rech/tbr/ump88gx/EJ_logs/%j.out run/wrapper.sb ${main} --repeat 1  ${common_params}"
    echo $script
    eval $script
}
